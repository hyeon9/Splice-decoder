...
outdir = args.input
files = [f for f in os.listdir(dir) if os.path.isfile(os.path.join(dir, f)) if f.endswith("JCEC.txt")]
...
for header,matsfile in enumerate(files):
    ...
    df["mPSI1"] = df["IncLevel1"].apply(cal_mean)
    df["mPSI2"] = df["IncLevel2"].apply(cal_mean)
    df["mIJC_SAMPLE_1"] = df["IJC_SAMPLE_1"].apply(cal_mean)
    df["mIJC_SAMPLE_2"] = df["IJC_SAMPLE_2"].apply(cal_mean)
    df["mSJC_SAMPLE_1"] = df["SJC_SAMPLE_1"].apply(cal_mean)
    df["mSJC_SAMPLE_2"] = df["SJC_SAMPLE_2"].apply(cal_mean)
    before = df.shape[0]
    
    ## Minimum support read options Update 250416 to fileter out DSEs with lower support read (< 5)
    df = df[((df["mIJC_SAMPLE_1"] >= 5) | (df["mIJC_SAMPLE_2"] >= 5)) &\
            ((df["mSJC_SAMPLE_1"] >= 5) | (df["mSJC_SAMPLE_2"] >= 5))]
    print(f"{before-df.shape[0]} {event_type} events were filtered by less support reads") 
    
    ## Make final output
    out = df[["long_ID","Event_type",geneid_type,"E1","E2","E3","E4","strand","FDR","IncLevelDifference","mPSI1","mPSI2"]].copy()
    
    ## Calculate dPSI group2 - group1
    out["IncLevelDifference"] = out["IncLevelDifference"] * (-1)

    ## FDR and PSI filtering
    out = out[(out["FDR"].astype(float) < FDR_cut) & \
              (np.abs(out["IncLevelDifference"].astype(float)) > PSI_cut)]

    if header > 0:
        out.to_csv(outdir+"rmat.csv",
                   sep=",",
                   mode="a",
                   index=None,
                   header=None)
    else:
        out.columns = ["long_ID","Event_type",args.geneid[0],"E1","E2","E3","E4","strand","FDR","dPSI_2_minus_1","mPSI1","mPSI2"]
        out.to_csv(outdir+"rmat.csv",
                   sep=",",
                   index=None)
# %%
