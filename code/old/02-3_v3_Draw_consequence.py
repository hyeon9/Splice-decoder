...
def Run(key, input_gene_name, *sub_splicing):
    ...
    def Comp_plot(canvas, exonpos, exon_color, *pos_info):
        ...
        if len(exonpos) > 0:  # If there are available exon coordinates
            ...
            def draw_exon(span, canvas, *domain):
                ...
                elif sub_splicing[0] in ["Alt_A3SS", "Alt_A5SS"]:
                    if len(domain) > 2 and exon_color != "#73ABFF":
                        canvas.fill_between(span, ylims['exon_min'], ylims['exon_max'],
                                            edgecolor="#000000", facecolor="#FFFFFF",
                                            zorder=4, linestyle=":", linewidth=1.5)
            ...
            def draw_plot(canvas):
                ...
                for i in range(len(exonpos)):   # For each exon block
                    ...
                    # Drawing the changed exon structure
                    elif len(pos_info) >= 3:
    ...
    def Make_query(final, pfam):
        ...
        int_final = final.copy()
        int_pfam = pfam.copy()
        if strand == "-":
            int_final[[2,1]] = final[[1,2]]*(-1)
            int_pfam[[2,1]] = pfam[[1,2]]*(-1)
            int_pfam[[9,8]] = pfam[[8,9]]*(-1)
        
        int_pfam = int_pfam[int_pfam[5]==strand]
        int_pfam[3] = int_pfam[3]+";"+int_pfam[4]+";"+int_pfam[6]
        for line in range(int_final.shape[0]):
            if line > 0:
                ## ADD strand specific domain assign and multiple domain case
                domain = int_pfam[(int_pfam[8]==int_final.iloc[line,1]) &
                            (int_pfam[9]==int_final.iloc[line,2])][[0,1,2,3]].drop_duplicates()   # Capture the overlapped domain with certain exon and remove duplicates
                intron = np.abs(int(int_final.iloc[line-1,2] - int(int_final.iloc[line,1])))    # (N-1)th exon end - Nth exon start
                diff.append(intron)
                new_line.append([int(int_final.iloc[line,1])-np.sum(diff)-zero,
                                int(int_final.iloc[line,2])-np.sum(diff)-zero])
                ...
                pre_domain = " "
                zero = int(int_final.iloc[line,1])  # The most left exon's start site
                new_line.append([int(int_final.iloc[line,1])-zero,
                                int(int_final.iloc[line,2])-zero])
                domain = int_pfam[(int_pfam[8]==int_final.iloc[line,1]) &
                            (int_pfam[9]==int_final.iloc[line,2])][[0,1,2,3]].drop_duplicates()
                    ...
                    if len(ref_bed) > 0:    # Markup changed regions
                        ...
                        for r,s in zip(ref_bed,sim_bed):
                            ...
                            r[1] != s[1]:
                                ...
                                event_pos_query = sim_bed.index(s)  # Position of sim DS
                                diffa = np.abs(s[0]-r[0])
                                diffb = np.abs(s[1]-r[1])
                                space = diffa + diffb
                        ...
                        def Prunning(domain_bed, dname, st, end, diff_exon, *space):
                            ...
                            for index in range(len(domain_bed)):
                                if domain_bed[index][0] > diff_exon[0]:    # If domain is located after diff exon
                                    domain_bed[index][0] = domain_bed[index][0]+space[0]
                                    domain_bed[index][1] = domain_bed[index][1]+space[0]
                        ...
                        if fig_input["event_type"].unique()[0] in ["EI","Alt_A3SS","Alt_A5SS"]: # Sim EI, Alt A3SS, and Alt A5SS > sim is longer
                            p_ref_domain, p_ref_dname, l_ref_domain = Prunning(ref_domain, ref_dname,
                                                                               fig_input["Start"].values[0], 
                                                                               fig_input["STOP"].values[0]+space,
                        ...
                        elif fig_input["event_type"].unique()[0] in ["ES","Ori_A3SS","Ori_A5SS"]:   # Sim ES, Ori A3SS, and Ori A5SS > sim is shorter
                            p_ref_domain, p_ref_dname, l_ref_domain = Prunning(ref_domain, ref_dname,
                                                                               fig_input["Start"].values[0], 
                            ...
                            p_sim_domain, p_sim_dname, l_sim_domain = Prunning(sim_domain, sim_dname,
                                                                               fig_input["simStart"].values[0], 
                                                                               fig_input["simSTOP"].values[0]+space,
                        ...
                        if len(p_ref_domain) > 0 and fig_on == "on":
                            if n_row == 0:
                                ...
                                if Make_fig == 1:
                                    ...
                                    if fig_input["event_type"].unique()[0] == "EI":
                                        ...
                                        new_bed = sim_bed.copy()    # Use Sim bed as template
                                        new_bed[event_pos_query] = ref_bed[event_pos_ref]   # add ref exon
                                        new_bed.insert(event_pos_query+1, [ref_bed[event_pos_ref][1], sim_bed[event_pos_query][1]]) # add DS part before the ref exon
                                        if fig_input["event_type"].unique()[0] == "Alt_A3SS":
                                            Comp_plot(ref_axs, new_bed, "#FF7878", fig_input["Start"].values[0], fig_input["STOP"].values[0]+a, event_pos_ref)
                                        else:
                                            Comp_plot(ref_axs, new_bed, "#FF7878", fig_input["Start"].values[0], fig_input["STOP"].values[0]+a, event_pos_ref+1)
                                            
                                    
                                    ## If sim splicing is skipped form, ref tx should have ref tx structure
                                    elif fig_input["event_type"].unique()[0] == "ES":
                                ...
                                if fig_input["event_type"].unique()[0] in ["ES","Ori_A3SS","Ori_A5SS"]:
                                    ...
                                    new_bed = ref_bed.copy()    # Use Ref bed as template
                                    new_bed[event_pos_query] = sim_bed[event_pos_ref]   # add sim exon
                                    new_bed.insert(event_pos_query+1, [sim_bed[event_pos_ref][1], ref_bed[event_pos_query][1]]) # add DS part
                                    if fig_input["event_type"].unique()[0] == "Ori_A3SS":
                                        Comp_plot(sim_axs, new_bed, "#73ABFF", fig_input["simStart"].values[0], fig_input["simSTOP"].values[0]+b, event_pos_query)
                                    elif fig_input["event_type"].unique()[0] == "Ori_A5SS":
                                        Comp_plot(sim_axs, new_bed, "#73ABFF", fig_input["simStart"].values[0], fig_input["simSTOP"].values[0]+b, event_pos_query+1)
                                    else:   # ES
